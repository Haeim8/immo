...
contract CANTORFI is Initializable, ERC721Upgradeable, ERC2981Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        // Set 4% royalties for OpenSea (400 basis points = 4%)
        _setDefaultRoyalty(_treasury, 400);
     ...
     * @dev Timestamp usage is SAFE: used for time-based sale deadlines (not critical randomness)
     * @dev Miners can only manipulate by ~15s, acceptable for day/week-long sales
     */
    function takePuzzle() external payable nonReentrant whenNotPaused {
        ...
        // Auto-close if sold out - MUST be before external call (reentrancy protection)
        bool shouldClose = info.puzzlesSold >= info.totalPuzzles;
        if (shouldClose) {
            info.isActive = false;
        }

        // Transfer payment to treasury (external call AFTER all state changes)
        // slither-disable-next-line low-level-calls
        // Low-level call is SAFE: CEI pattern used, protected by nonReentrant, state finalized
    ...
    function closeSale() external onlyFactoryOrTeam {
        // slither-disable-next-line timestamp
        // Timestamp is SAFE: used for time-based sale deadline enforcement
    ...
    function depositRewards() external payable onlyFactoryOrTeam whenNotPaused {
        ...
        // Divide-before-multiply is INTENTIONAL: we need integer division to track remainder
        // This is the ONLY way to calculate exact remainder for fair distribution
        ...
        uint256 distributed = rewardPerPuzzle * puzzlesSnapshot; // Intentional: track remainder
    ...
    function claimRewards(uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        // slither-disable-next-line timestamp
        // Timestamp is SAFE: used for rate limiting (1h cooldown), manipulation insignificant
    ...
    function createProposal(
        ...
        // slither-disable-next-line timestamp
        // Timestamp is SAFE: used for governance voting deadlines
        ...
        // slither-disable-next-line timestamp
        emit ProposalCreated(proposalId, title, block.timestamp + votingDuration);
    ...
    function castVote(uint256 proposalId, uint256 tokenId, bool vote) external whenNotPaused {
        ...
        // Auto-close if deadline passed
        // slither-disable-next-line timestamp
        // Timestamp is SAFE: used for governance voting deadline enforcement
    ...
    function claimCompletion(uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        // slither-disable-next-line low-level-calls
        // Low-level call is SAFE: CEI pattern used, protected by nonReentrant, NFT burned before transfer
    ...
    function canClaimRewards(uint256 tokenId) external view returns (bool) {
        ...
        // slither-disable-next-line timestamp
        // Timestamp is SAFE: view function for UI, cooldown check only
    ...
    /**
     * @notice Declares support for ERC721, ERC2981 (royalties) and other interfaces
     * @dev Required for OpenSea royalty enforcement
     */
